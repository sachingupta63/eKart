...
const PlaceOrderScreen = () => {
    ...
    //Calculating Price
    cart.itemsPrice = Number(cart.cartItems.reduce((acc, item) => acc + item.qty * item.price, 0)).toFixed(2)

    cart.shippingPrice = cart.itemsPrice > 100 ? 0 : 40

    cart.taxPrice = Number((.28 * cart.itemsPrice).toFixed(2))
                                ...
                                {cart.cartItems.map((item, index) => (
                                    <ListGroup.Item key={index}>
                                        <Row>
                                            ...
                                            <Col md={4}>
                                                {item.qty}x<i class="fa fa-inr"></i>{item.price}=<i class="fa fa-inr"></i>{item.qty * item.price}
