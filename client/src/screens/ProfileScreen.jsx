...
import { getOrderDetails, listMyOrder } from '../actions/orderActions'
...
const ProfileScreen = () => {
    ...
    const { loading: loadingOrder, orders, error: errorOrder } = orderMyList
                    ...
                    <tbody>
                        {orders.map(order => (
                            <tr key={order._id} className='text-center'>
                                ...
                                <td>{order.isDelivered ? order.deliveredAt.substr(0, 10) : (<i className='fas fa-times' style={{ color: 'red' }}></i>)}</td>
                                <td >
                                    <LinkContainer to={`/order/${order._id}`}>
                                        <Button onClick={() => dispatch(getOrderDetails(order._id))} className='btn-sm' variant='light'>Details</Button>
